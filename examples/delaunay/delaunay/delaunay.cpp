...
int main() {
  ...
  {  // Print usage.
    cout << "This examples computes and renders the Delaunay triangulation of\n"
            "custom or randomly chosen points. For the triangle marked by the\n"
            "cursor, the circumcircle is drawn.\n\n";
    constexpr int first_space = 20;
    constexpr int space = 50;
    cout << left;
    cout << "usage:\n"
         << setw(first_space) << "esc" << setw(space) << "Quit the program."
         << '\n'
         << setw(first_space) << "space" << setw(space)
         << "Uniformly generate random points in view." << '\n'
         << setw(first_space) << "left click + move" << setw(space)
         << "Move the viewport along the cursor movement." << '\n'
         << setw(first_space) << "right click" << setw(space)
         << "Add point at position of cursor." << '\n'
         << setw(first_space) << "mouse wheel" << setw(space)
         << "Zoom in or out." << '\n'
         << '\n';
    cout << right;
  }
  ...
  while (window.isOpen()) {
    ...
    for (const auto& t : triangulation.triangles) {
      if (delaunay::intersection({{{triangulation.points[t.pid[0]].x,
        ...
        // Draw circumcircle.
        const auto c =
            delaunay::circumcircle({{{triangulation.points[t.pid[0]].x,
                                      triangulation.points[t.pid[0]].y},
                                     {triangulation.points[t.pid[1]].x,
                                      triangulation.points[t.pid[1]].y},
                                     {triangulation.points[t.pid[2]].x,
